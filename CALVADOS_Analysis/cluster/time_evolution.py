...
def analyze_large_clusters(top, traj, contact_threshold=6.0, n_chains=500, chain_length=163,
                           n_frames=500, process_interval=50, min_cluster_size=20):
    """
    Analyze the number of large clusters over time.
    ...
    - min_cluster_size: Minimum cluster size to be considered.
    """
    output_dir = os.path.join(os.path.dirname(traj), "large_cluster_evolution")
    ...
    default_box = np.array([2000.0, 2000.0, 2000.0])  # 200nm

    total_frames = len(u.trajectory[:n_frames])

    for ts in tqdm(u.trajectory[:n_frames], desc="Analyzing large clusters"):
        ...
        large_clusters = cluster_sizes[cluster_sizes >= min_cluster_size]

        frames.append(ts.frame)
        large_cluster_counts.append(len(large_clusters))

    plt.figure()
    plt.plot(frames, large_cluster_counts, marker='o')
    plt.xlabel("Frame", fontname='Times New Roman')
    plt.ylabel(f"Clusters (>{min_cluster_size} chains)", fontname='Times New Roman')
    plt.title("Large Cluster Evolution", fontname='Times New Roman')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "large_cluster_evolution.png"))
    plt.close()
